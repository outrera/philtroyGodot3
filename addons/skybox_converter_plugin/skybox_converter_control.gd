...
func _file_open(file_path):
	...
	if res:
		get_node("ImagePreview/TextureRect").set_texture(res)
		make_images()
...
func make_images():
	var rf = get_node("ImagePreview/TextureRect").get_texture()
	var texture = get_node("ImagePreview/TextureRect").get_texture()
...
func _on_matrix1_released():
	...
	mode = 0
	get_node("ImagePreview/TextureRect/grid").set_mode(mode)
	make_images()
